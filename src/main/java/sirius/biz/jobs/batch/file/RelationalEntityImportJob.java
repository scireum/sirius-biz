...
public class RelationalEntityImportJob<E extends BaseEntity<?> & ImportTransactionalEntity, Q extends Query<Q, E, ?>>
    ...
    @Override
    protected void executeForStream(String filename, Producer<InputStream> inputStreamSupplier) throws Exception {
        importTransactionHelper.start();
        try (InputStream in = inputStreamSupplier.create()) {

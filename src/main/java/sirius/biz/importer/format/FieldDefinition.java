...
public class FieldDefinition {
    ...
    /**
     * Adds a remark to be shown for this field, e.g. to explain the expected format of a value.
     * <p>
     * Remarks are purely informative and are listed along with the remarks generated by the
     * {@linkplain #withCheck(ValueCheck) checks} of this field.
     *
     * @param remark the remark to add, which will be {@link sirius.kernel.nls.NLS#smartGet(String) auto translated}
     *               when the remarks are {@linkplain #getRemarks() rendered}
     * @return the field itself for fluent method calls
     */
    public FieldDefinition addRemark(String remark) {
        if (Strings.isFilled(remark)) {
    ...
    public List<String> getRemarks() {
        ...
        remarks.stream().map(NLS::smartGet).forEach(result::add);
        if (!aliases.isEmpty()) {
